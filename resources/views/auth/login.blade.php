...
@section('title', 'Login')
@section('global')
<main class="px-3 md:px-5 py-2 md:py-3 lg:py-6">
   <div class="grid grid-cols-1 md:grid-cols-2">
      <form action="{{route('login.store')}}" method="post">
         ...
         <div class="flex flex-col py-5 md:px-5 lg:px-12">
            ...
            <x-site.input type="password" name="password" model="default" placeholder="********" required />
            <a href="" class="text-sm text-blue-400 text-right font-light my-3">Lost password?</a>
            <x-site.button class="bg-[#4965ab] mt-1 mx-auto block" fill="none" value="Log in" />
            <p class="text-sm font-light text-slate-600 dark:text-slate-200 mt-2">Not registered? <a class="text-blue-400" href="{{route('register')}}">Create account</a></p>
         </div>
      </form>
